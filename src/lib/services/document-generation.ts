...
type IdentityAssets = Awaited<
  ReturnType<typeof DocumentIdentityService.generateIdentityAssets>
>;
...
export class DocumentGenerationService {
  static async generateDocument<TPayload extends object>(
    ...
    const { context, identityAssets } =
      await this.buildTemplateContext(documentType, payload);

    // ✅ Use writable temp storage on Vercel
    const assetRoot = path.join(
      "/tmp",
      "documents",
      this.toFolderName(documentType),
    );
    ...
    const identity: ReactPdfIdentityView = {
      ...
      qrDataUri: `data:${identityAssets.qrCodeMimeType};base64,${identityAssets.qrCodeBuffer.toString(
        "base64",
      )}`,
      barcodeDataUri: `data:${identityAssets.barcodeMimeType};base64,${identityAssets.barcodeBuffer.toString(
        "base64",
      )}`,
    };
    ...
    const documentElement = buildReactPdfDocument(
      documentType,
      payload,
      templateContext,
      identity,
  ...
  private static async buildTemplateContext<TPayload extends object>(
    documentType: SupportedDocumentType,
    payload: TPayload,
  ): Promise<{ context: TemplateContext; identityAssets: IdentityAssets }> {
    const normalizedIdentifier =
      DocumentIdentityService.normalizeRegistrationIdentifier(
        this.extractRegistrationIdentifier(payload),
      );

    const identityAssets =
      await DocumentIdentityService.generateIdentityAssets(
  ...
  static buildDocumentReference(input: {
    ...
    const normalized = this.normalizeReferenceIdentifier(
      input.registrationIdentifier,
    );

    const sequence = normalized.match(/-(\d{5})$/)?.[1] ?? "00001";

    const year = input.generatedAt
      ? new Date(input.generatedAt).getFullYear()
  ...
  private static normalizeReferenceIdentifier(
    registrationIdentifier: string,
  ): string {
    return registrationIdentifier.trim().toUpperCase();
  }

  private static extractRegistrationIdentifier<TPayload extends object>(
    payload: TPayload,
  ): string {
  ...
  private static extractGeneratedAt<TPayload extends object>(
    payload: TPayload,
  ): string {
    ...
    return typeof value === "string" && value.trim()
      ? value
      : new Date().toISOString().split("T")[0] ?? "2026-07-22";
  }
