...
export default function NewProjectPage() {
  ...
  // fetch helpers are declared below; call them after definitions
  ...
  const fetchWorkers = async () => {
    try {
      const response = await fetch("/api/supervisor/workers?limit=100");
      if (response.ok) {
        const data = await response.json();
  ...
  const filteredLearners =
    formData.departmentId === "no-department"
      ? learners
      : learners.filter(
          (learner) => learner.department?.id === formData.departmentId,
        );
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Create a new project and assign learners for competency milestones
            </p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Fill in the project information and assign learners.
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <div className="space-y-2">
                  ...
                  {formData.departmentId !== "no-department" && (
                    <p className="text-sm text-muted-foreground">
                      Showing learners from the selected department only.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Select learners for this project.
                    </p>
                  </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {filteredLearners.map((learner) => (
                    ...
                    >
                      ...
                      />
                      <span>
                        {learner.user.name} ({learner.regNumber})
                      </span>
                    </label>
                  ))}
                  {filteredLearners.length === 0 && (
                    <div className="text-sm text-muted-foreground">
                      No learners available yet for your selection.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Select ERP-synced workers for this project.
                    </p>
                  </div>
                  <div className="text-sm text-muted-foreground">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  {workers.length === 0 && (
                    <div className="text-sm text-muted-foreground">
                      No workers available yet. Synchronize workers from ERP.
                    </div>
                  )}
                </div>
              </div>
