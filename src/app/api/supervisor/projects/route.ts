...
export async function POST(request: NextRequest) {
  try {
    ...
    if (selectedLearnerIds.length > 0 && validatedData.departmentId) {
      ...
      const invalidLearner = learners.find((learner) => learner.departmentId !== validatedData.departmentId)
      if (invalidLearner) {
        return NextResponse.json({ error: 'Selected learners must belong to the chosen department' }, { status: 400 })
      }
    }
